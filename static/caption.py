...
def creating_text_help(name_user):
    caption = f'<b>Привет👋, {name_user}</b>\n' \
              ...
              f'<b>Инструкция</b>\n' \
              f'💜Чтобы начать отслеживать товар, скиньте ссылку на товар в чат с ботом.\n' \
              f'💜Чтобы прекратить отслеживать товар, перейдите в "Мои товары" и нажмите кнопку "Удалить" под товаром.\n' \
              f'❗️Бот показывает цену без учёта вашей личной скидки\n' \
              f'❗️Бот показывает цену без учёта региональной стоимости\n' \
              f'(по умолчанию цена для г. Москва)\n\n' \
              f'💜У каждого пользователя есть личный кабинет в котором можно:\n' \
              f'✅посмотреть сколько ваших товаров отслеживается\n' \
